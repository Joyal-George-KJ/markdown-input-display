...
export default function MarkdownInput({
    ...
    const handleClick = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const formatter = markdownShortcuts[key].transform;

        if (!formatter) return;

        const newMd = md.slice(0, start) + formatter(selected) + md.slice(end);
    ...
    return (
        <div style={mergedStyles.wrapper}>
            <div style={mergedStyles.editor}>
                <div style={mergedStyles.toolbar}>
                    <div style={mergedStyles.toolbarButtons}>
                        {Object.keys(markdownShortcuts).map((val, ind) => (
                            <button
                                key={ind}
                                onClick={handleClick}
                                id={val}
                                style={mergedStyles.toolbarBtn}
                                onMouseOver={(e) => {
                            ...
                            >
                                {markdownShortcutIcons[val]?.icon()}
                            </button>
                        ))}
                    </div>
                    <div>
                        ...
                        >
                            {markdownShortcutIcons.preview?.icon()}
                        </button>
                    </div>
                </div>
                ...
                {previewToggle && (
                    <MarkdownPreview
                        onClose={() => setPreviewToggle(false)}
                        md={md}
                    />
                )}

...
export function customMarkdownToHTML(md: string): string {
  ...
  html = html.replace(/\*\*(.*?)\*\*/gim, (_, c) => `<strong>${escapeHTML(c)}</strong>`);
  html = html.replace(/\*(.*?)\*/gim, (_, c) => `<em>${escapeHTML(c)}</em>`);
  html = html.replace(/^> ?(.*)$/gim, (_, c) => `<blockquote>${escapeHTML(c)}</blockquote>`);
  html = html.replace(/^\d+\.+(.*)/gim, (_, c) => `<ol><li>${escapeHTML(c)}</li></ol>`);
  html = html.replace(/^\-+(.*)/gim, (_, c) => `<ul><li>${escapeHTML(c)}</li></ul>`);
